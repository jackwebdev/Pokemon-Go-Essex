...
S2.latLngToNeighborKeys = S2.S2Cell.latLngToNeighborKeys = function (lat, lng, level) {
  return S2.S2Cell.FromLatLng({ lat: lat, lng: lng }, level).getNeighbors(1).map(function (cell) {
    return cell.toHilbertQuadkey();
  });
};
S2.S2Cell.prototype.getNeighbors = function(nlevel) {
  ...
  var neighbors = [];
  for (var a = -nlevel; a <= nlevel; a++) {
    for (var b = -nlevel; b <= nlevel; b++) {
      neighbors.push( fromFaceIJWrap(face, [i-a,j-b], level));
    }
  }
...
S2.latLngToChildrenKeys = S2.S2Cell.latLngToChildrenKeys = function (lat, lng, plevel, clevel) {
  return S2.S2Cell.FromLatLng({ lat: lat, lng: lng }, plevel).getChildren(clevel).map(function (cell) {
    return cell.toHilbertQuadkey();
  });
};
S2.S2Cell.prototype.getChildren = function(clevel) { // children level, number of rings around parent cell
  if (clevel - this.level > 3)
    return this.getChildrenRing(clevel, 0);
  else
    return this.getChildrenRing(clevel, 1);
}
S2.S2Cell.prototype.getChildrenRing = function(clevel, pring) { // children level, number of rings around parent cell
  ...
  var s2cells = {};
  s2cells['neighbors'] = this.getNeighbors(pring);
  s2cells['children'] = [];
  var diff = clevel-level;

  if (diff <= 0) return s2cells;

  var num_children = Math.pow(4,diff);

  for (var a = 0; a < s2cells['neighbors'].length; a++) {
    var id = s2cells['neighbors'][a].toHilbertQuadkey();
    for (var b = 0; b < num_children; b++) {
      var lstr = Long.fromValue(b).toString(4); // value of b as a base-4 string
      var qstr = (Array(diff+1).join("0")+lstr).substring(lstr.length);
      s2cells['children'].push( S2.S2Cell.FromHilbertQuadKey(id + qstr) );
    }
  }
...
/*
    ...
    var square = (2*s2ring+1)*(2*s2ring+1);
console.log(s2);
console.log(s2.getNeighbors(1));
console.log(s2.toHilbertQuadkey());
var id = s2.toHilbertQuadkey();
console.log(id + '0');
console.log(id + '1');
console.log(id + '2');
console.log(id + '3');
console.log(S2.S2Cell.FromHilbertQuadKey(id + '0'));
console.log(S2.S2Cell.FromHilbertQuadKey(id + '1'));
console.log(S2.S2Cell.FromHilbertQuadKey(id + '2'));
console.log(S2.S2Cell.FromHilbertQuadKey(id + '3'));
*/
  //return neighbors;
